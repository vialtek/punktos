...
static inline bool TrimRange(O& offset, L& len, O trim_to_len) {
    static_assert(sbl::numeric_limits<O>::is_signed == false, "TrimRange requires unsigned type O");
    static_assert(sbl::numeric_limits<L>::is_signed == false, "TrimRange requires unsigned type L");
