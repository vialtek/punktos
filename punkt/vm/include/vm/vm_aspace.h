...
class VmAspace : public sbl::RefCounted<VmAspace> {
public:
    ...
    static sbl::RefPtr<VmAspace> Create(uint flags, const char* name);
    ...
    status_t MapObject(sbl::RefPtr<VmObject> vmo, const char* name, uint64_t offset, size_t size,
    ...
    sbl::RefPtr<VmRegion> FindRegion(vaddr_t vaddr);
...
private:
    ...
    status_t AddRegion(sbl::RefPtr<VmRegion> r);
    sbl::RefPtr<VmRegion> AllocRegion(const char* name, size_t size, vaddr_t vaddr,
    ...
    sbl::DoublyLinkedList<VmRegion> regions_;
