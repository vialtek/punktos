...
sbl::RefPtr<VmRegion> VmRegion::Create(VmAspace& aspace, vaddr_t base, size_t size,
                                         uint arch_mmu_flags, const char* name) {
    return sbl::AdoptRef(new VmRegion(aspace, base, size, arch_mmu_flags, name));
}
...
status_t VmRegion::SetObject(sbl::RefPtr<VmObject> o, uint64_t offset) {
    DEBUG_ASSERT(magic_ == MAGIC);
